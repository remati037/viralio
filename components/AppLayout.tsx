...
import { canCreateTask, getRemainingTasks, getTierLimits } from '@/lib/utils/tierRestrictions'
import type { TaskInsert, UserTier } from '@/types'
import { ClipboardList, Layout, Menu, Play, Plus, Shield, Trello, User, X } from 'lucide-react'
...
export default function AppLayout({ userId, children }: { userId: string; children: React.ReactNode }) {
  ...
  const { tasks, loading: tasksLoading, createTask, addInspirationLink } = useTasks(userId)
  ...
  const showLoader = mounted && (profileLoading || tasksLoading || competitorsLoading)
        ...
        <div className="flex">
          ...
          >
                ...
                <div>
                  ...
                  {profile?.role === 'admin' && (
                    <Link
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 mb-2 ${isActive('/admin')
                  ...
                  <Link
                    href="/planner"
                    onClick={() => setIsSidebarOpen(false)}
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 mb-2 ${isActive('/planner')
                  ...
                  <Link
                    href="/competitors"
                    onClick={() => setIsSidebarOpen(false)}
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 mb-2 ${isActive('/competitors')
                        ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/20'
                        : 'text-slate-400 hover:bg-slate-800 hover:text-white'
                      }`}
                  >
                  ...
                  <Link
                    href="/casestudy"
                    onClick={() => setIsSidebarOpen(false)}
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 mb-2 ${isActive('/casestudy')
                        ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/20'
                        : 'text-slate-400 hover:bg-slate-800 hover:text-white'
                      }`}
                  >
            ...
            <div className="absolute bottom-0 w-full p-6 border-t border-slate-800">
              ...
              >
                <div
                  className={`w-10 h-10 rounded-full bg-blue-600 flex items-center justify-center font-bold text-white ${isActive('/profile') ? 'ring-2 ring-blue-500' : ''
                    }`}
                >
          ...
          {/* Main Content */}
          <main className="flex-1 p-4 lg:p-8 overflow-x-hidden h-screen overflow-y-auto">{children}</main>
        </div>
            ...
            onSaveToPlan={async (taskData, inspirationLinks) => {
              ...
              if (inspirationLinks && inspirationLinks.length > 0 && result.data) {
                for (const linkData of inspirationLinks) {
                  await addInspirationLink(result.data.id, linkData.link, linkData.displayUrl, linkData.type)
                }
              }
